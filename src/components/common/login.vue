...
<div>
  <!-- 登录框 -->
  <!-- <Modal v-model="loginBox" width="440" class="login-wrap">
    <p slot="header">您还未登录，请登录查看更多详情！</p>
    ...
    <div slot="footer">
      <Button type="success" size="large" long :loading="modal_loading" @click="loginSuccess">登录</Button>
      <p>您还未注册？请点击 <a @click="handleRegister">快速注册</a></p>
    </div>
  </Modal> -->
  <!-- 注册框 -->
  <Modal v-model="registerBox" width="440" class="login-wrap">
    <div class="login-content">
      <h2>会员注册</h2>
  ...
  <Modal v-model="agreementBox" width="1200" class="agreement-wrap">
    ...
    <div class="agreement-content">
      <p>欢迎阅读涉药运输服务平台-医药四方云（以下简称“平台”）所提供的协议内容。本平台中国医药商业协会社会物流医药分会、深圳传世般若科技有限公司联合推出，由深圳传世般若科技有限公司提供技术支持与主体运营的医药社会物流信息平台。</p>
...
export default {
  ...
  methods: {
    ...
    //   setTimeout(() => {
    //     this.$http
    //       .post("api/LoginValidate/LoginValidate", {
    ...
    //           self.$store.commit('logOn', {Name:self.loginName});
    //         } else {
    //           self.$Message.warning("用户名密码不一致,请重新输入");
    //           self.modal_loading = false;
    //         }
    //       });
    //   }, 2000);
    ...
    OutVerifyCode() {
      ...
      if (!this.Mobile || !testTel.test(this.Mobile)) {
        this.$Message.warning("请输入请输入您的正确手机号码！");
        return false;
      }
      var Num = "";
      for (var i = 0; i < 4; i++) {
      ...
      this.$http
        .post("/api/SendMessagesApi/SendSMS", {
          Mobile: this.Mobile,
          VerifyCode: Num
        })
        .then(function(rs) {
    ...
    ValidateControl() {
      ...
      if (!this.rememberPwd) {
        this.$Message.warning("请勾选已阅读并接受！");
        return false;
      }
      return true;
    },
    ...
    Save() {
      ...
      if (this.ValidateControl()) {
        this.$http
          .post("api/CtaApiController/MemberRegist", {
